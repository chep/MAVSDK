...
mavlink_dance_fram_ftp_t DanceData::parse(mavlink_dance_fram_ftp_t& req)
{
    mavlink_dance_fram_ftp_t ans;
    ...
    switch (req.opcode) {
        case MAV_DANCE_FRAM_OPCODES_WRITE:
            if (_data.size() < req.offset + req.len)
                _data.resize(req.offset + req.len);
            memcpy(_data.data() + req.offset, req.payload, req.len);
            ans.payload[0] = ACK;
            _is_valid = false; // reset flag because CRC could be wrong now
            break;

        case MAV_DANCE_FRAM_OPCODES_CRC32: {
            uint32_t crc32;
            memcpy(&crc32, req.payload, sizeof(uint32_t));
            crc32 = ntohl(crc32);

            if (compare_crc(crc32))
                ans.payload[0] = ACK;
        } break;
        ...
        default:
            LogWarn() << "FTP fram opcode not handled " << req.opcode;
            break;
    }
...
bool DanceData::compare_crc(uint32_t crc)
{
    uint32_t crc32 = 0;
    ...
    for (auto d : _data)
        crc32 = crc32_tab[(crc32 ^ d) & 0xff] ^ (crc32 >> 8);
